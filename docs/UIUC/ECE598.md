...
这就是VAE的ELBO（证据下界）：
\[
L(\theta,\phi;x)=\log p(x) \geq E_{q(z|x)}[\log p(x|z)] - D_{KL}(q(z|x) \| p(z)).
\]
...
1.   Blur with Gaussian kernel of variance: $\sigma_0$, $\sigma_1$...., $\sigma_k$
2.   Sample random point y
3.   $De_\theta$(noise point y)-> $E[x|y]$
4.   formula $\nabla_y \log p(y)=\frac{De_\theta(y)-y}{\sigma_k^2}$
5.   Lagevin Dy for s steps using $\sigma_k^2$
6.   Say you are at $y_{t+s}$
7.   $\nabla_y \log p(y)=\frac{De_\theta(y_{t+s})-y}{\sigma_{k-1}^2}$
8.   Lagevin Dy for s steps using $\sigma_{k-1}^2$
9.   .....
...
<img src="./assets/image-20251006161559296.png" alt="image-20251006161559296" style="zoom: 33%;" />

All I need is $\nabla_x \log p(x|c)$, score function

<img src="./assets/image-20251006163533653.png" alt="image-20251006163533653" style="zoom: 33%;" />
...
![image-20251015161338177](./assets/image-20251015161338177.png)
...
$$\mathbb{E}_{x,z}[\cdot] = \mathbb{E}_x \Big[ \mathbb{E}_{z|x}[\cdot] \Big]$$

Chain rule of probability: $p(x, z) = p(z|x)p(x)$ 

$$\mathbb{E}_{x,z}[f(x,z)] = \iint f(x,z) p(x,z) \, dz \, dx = \iint f(x,z) \underbrace{p(z|x)p(x)}_{\text{substituted}} \, dz \, dx = \int \left[ \int f(x,z) p(z|x) \, dz \right] p(x) \, dx$$
