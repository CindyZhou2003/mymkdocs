...
-   **William Labov**: A linguist, not a computer scientist. His work showed that language variation isn't random but is correlated with social factors. This is relevant to AI because it highlights the importance of **context and bias** in language data.
...
-   **Softmax**: Softmax is a differentiable function 可微函数 that approximates this discrete behavior. It's best thought of as a version of argmax.
    ...
    -   Suppose we have a sequence of classifier outputs, one per class. $v_1,…,v_n$. Softmax maps 映射each $v_i$ to $\frac{e^{v_i}}{∑_ie^{v_i}}$. The exponentiation accentuates the contrast between the largest value and the others, and forces all the values to be positive. The denominator normalizes all the values into the range [0,1].
...
-   **Limitations** of perceptrons and ways to address them
    ...
    -   The decision boundary can only be a line.
        ...
        >   -   massage input features to make the boundary linear按摩输入特征使边界线性化
...
The phrasal recognition dataset consists of 17 phrases and 8 objects. There are 2769 images(822 negative images) in this dataset and on average 120 images per category. In total there are 5067 bounding boxes (1796 for visual phrases+3271 for 1747 objects) in this dataset.
