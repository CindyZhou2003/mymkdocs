...
>   One approach is to use **grid-wide barrier synchronizations** with cooperative groups and scan the partial sums with a single thread block
>
>   Limits the number of thread blocks that can execute, thereby the size of the array that can be scanned
>
>   Another approach is to use **unidirectional synchronization** to pass the partial sums from earlier thread blocks to later thread blocks
