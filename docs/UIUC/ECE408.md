...
**Threads perform atomic operations serially**. 串行
...
-   Synchronize across blocks 跨块同步 to consolidate the segments (covered later)
...
**TF32** shares the same exponent format as FP32, and the mantissa format is simply a less precise version of FP32’s mantissa bits, so any TF32 value can also be stored in a float variable.
