...
Vector Processing Unit: a vector of same instructions done in parallel
...
![image-20260219160245383](./assets/image-20260219160245383.png)
...
-   **Reduces memory consumption** 
    ...
    -   Enables larger models, mini batches or inp

Pure 16 disadvatages
...
**When to Choose FP16 (Compute-Bound Operations)**

We use FP16 for operations that are mathematically safe to compute at lower precision and that consume the vast majority of your training time.

-   **Matrix Multiplications (GEMMs):** Linear layers, fully connected layers, and the QKV (Query, Key, Value) projections in attention mechanisms.
-   **Convolutions:** Standard image processing filters. (e.g., relu, tanh, add, sub, mul)
-   **Why FP16?** These operations are highly parallelizable and compute-heavy. Modern GPU architectures (specifically Tensor Cores) are purpose-built to accelerate FP16 matrix math significantly faster than FP32. Furthermore, using FP16 halves the memory bandwidth required to load those massive weight tensors from GPU memory to the compute cores.

**When to Choose FP32 (Memory-Bound & Sensitive Operations)**

We use FP32 for operations that require a larger dynamic range. FP16 can only represent numbers up to roughly 65,504, and its precision degrades with very tiny fractions.

-   **Reductions and Normalizations:** Operations that sum over large arrays—such as Softmax, Layer Normalization, or Batch Normalization(Element-wise operations where |f(x)| >> |x|, e.g., exp, log, pow)—are highly prone to overflow if computed in FP16. They should remain in FP32.
-   **Loss Computation:** Calculating the final loss must be done in FP32 to ensure the resulting gradients are accurate before they are propagated backward through the network.
-   **Optimizer States and Master Weights:** During the weight update step (e.g., using Adam), the gradient updates are often incredibly small. If you try to add a tiny FP16 gradient to an FP16 weight, the update might round down to zero, stalling your training. To prevent this, a "master copy" of the weights and the optimizer's momentum states are kept in FP32.

In Practice: Automatic Mixed Precision (AMP)

In modern deep learning workflows, you rarely have to write custom CUDA kernels or manually cast tensors layer-by-layer. Frameworks like PyTorch and TensorFlow feature Automatic Mixed Precision (AMP), which maintains an internal whitelist and blacklist of operations, automatically casting them to the appropriate precision on the fly.
...
![image-20260303154726494](./assets/image-20260303154726494.png)
...
Standard quantization strategy(int8) leads to catastrophic accuracy drop
